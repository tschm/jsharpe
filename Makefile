...
# Repo-owned addition to the generated shim. The `%:` comment below claims "a *file*
# sharing a task's name would shadow it -- none do", which is not true here: this repo has
# a `book/` directory, and `LICENSE` matches the `license` task on a case-insensitive
# filesystem (every macOS checkout). A pattern-rule target that exists as a file is subject
# to make's mtime comparison against its `$(UVX)` prerequisite, so `make book` and
# `make license` print "is up to date" and run nothing as soon as the path is newer than
# the uvx binary -- silently, with exit 0. Naming them phony removes the file from the
# comparison. A bare `.PHONY` is not enough: it creates a recipe-less explicit target,
# which beats the match-anything rule and yields "Nothing to be done" -- so the recipe
# has to be restated here. Re-check this list after `rhiza-task shim`
# regenerates the file, and whenever a new top-level path lands.
.PHONY: book license
book license: $(UVX)
	@$(UVX) $(RHIZA_TASK) $@
