    ...
    *,
    ...
    for k in range(2, max_clusters + 1):
        ...
        for _ in range(retries):
            ...
            kmeans.fit(
                D
            )  # Use the distances as features, i.e., compute distances between columns of the distance matrix
...
def minimum_track_record_length(
    ...
    *,
    ...
    """
    return (
        sharpe_ratio_variance(SR0, T=1, gamma3=gamma3, gamma4=gamma4, rho=rho, K=1)
        * (scipy.stats.norm.ppf(1 - alpha) / (SR - SR0)) ** 2
    )
...
def deflated_sharpe_ratio(
    ...
    if cluster:
        ...
        # Number of clusters
        number_of_trials, _qualities, clusters = number_of_clusters(C)
        assert number_of_trials == clusters.max() + 1

        variance, SRs, _y = variance_of_the_clustered_trials(X, clusters)
        ...
        ps = np.array(
            [1 - probabilistic_sharpe_ratio(u, SR0=0, T=X.shape[0], gamma3=gamma3, gamma4=gamma4) for u in SRs]
        )
    ...
    if verbose:
        ...
        print(f"DSR               = {DSR:.3f}")
        print(
            f"1 - DSR           = {1 - DSR:.3f} {'***' if DSR > 0.999 else '**' if DSR > 0.99 else '*' if DSR > 0.95 else '.' if DSR > 0.9 else ''}"
        )
        ...
        print(
            f"p (Holm)          = {adjusted_p_values_holm(ps).min():.3f}  (the *minimum* Bonferroni and Holm p-values are the same)"
        )
        ...
        r.append(
            {
                ...
                "p": p,
                "c": c,
                "FDP": np.sum((H == 0) & (c < X)) / (1e-100 + np.sum(c < X)),
            }
        )
    r = pd.DataFrame(r)
...
def test_numeric_example():
    ...
    for rho in [0, 0.2]:
        ...
        print(f"SR                     = {SR:.3f}")
        print(
            f"σ_SR                   = {math.sqrt(sharpe_ratio_variance(SR=mu / sigma, gamma3=gamma3, gamma4=gamma4, rho=rho, T=T)):.3f} (non-Gaussian)"
        )
        print(
            f"σ_SR                   = {math.sqrt(sharpe_ratio_variance(SR=mu / sigma, gamma3=0, gamma4=3, T=T)):.3f} (Gaussian, iid)"
        )
        print(
            f"MinTRL                 = {minimum_track_record_length(SR=mu / sigma, SR0=0, gamma3=gamma3, gamma4=gamma4, rho=rho, alpha=alpha):.3f}"
        )
        print(
            f"MinTRL(SR0=.1)         = {minimum_track_record_length(SR=mu / sigma, SR0=0.1, gamma3=gamma3, gamma4=gamma4, rho=rho, alpha=alpha):.3f}"
        )
        print(
            f"p = 1 - PSR(SR0=0)     = {1 - probabilistic_sharpe_ratio(SR=mu / sigma, SR0=0, T=T, gamma3=gamma3, gamma4=gamma4, rho=rho):.3f}"
        )
        print(
            f"PSR(SR0=0)             = {probabilistic_sharpe_ratio(SR=mu / sigma, SR0=0, T=T, gamma3=gamma3, gamma4=gamma4, rho=rho):.3f}"
        )
        print(
            f"PSR(SR0=.1)            = {probabilistic_sharpe_ratio(SR=mu / sigma, SR0=0.1, T=T, gamma3=gamma3, gamma4=gamma4, rho=rho):.3f}"
        )
        print(f"SR0                    = {SR0:.3f}")
        print(
            f"SR_c                   = {critical_sharpe_ratio(SR0, T, gamma3=0.0, gamma4=3.0, rho=0, alpha=alpha):.3f} (Gaussian, iid)"
        )
        print(
            f"SR_c                   = {critical_sharpe_ratio(SR0, T, gamma3=gamma3, gamma4=gamma4, rho=rho, alpha=alpha):.3f} (non-Gaussian -- unchanged if iid, SR0=0)"
        )
        print(f"SR1                    = {SR1:.3f}")
        print(
            f"Power = 1 - β          = {sharpe_ratio_power(SR0=SR0, SR1=SR1, T=T, gamma3=gamma3, gamma4=gamma4, rho=rho, alpha=alpha):.3f}"
        )
        print(
            f"β                      = {1 - sharpe_ratio_power(SR0=SR0, SR1=SR1, T=T, gamma3=gamma3, gamma4=gamma4, rho=rho, alpha=alpha):.3f}"
        )
        print(f"P[H1]                  = {p_H1:.3f}")
        print(
            f"pFDR = P[H0|SR>SR_c]   = {pFDR(p_H1, alpha, 1 - sharpe_ratio_power(SR0=SR0, SR1=SR1, T=T, gamma3=gamma3, gamma4=gamma4, rho=rho, alpha=alpha)):.3f}"
        )
        print(
            f"oFDR = P[H0|SR>SR_obs] = {oFDR(SR=mu / sigma, SR0=SR0, SR1=SR1, T=T, p_H1=p_H1, gamma3=gamma3, gamma4=gamma4, rho=rho):.3f}"
        )
        ...
        print(f"DSR                       = {DSR:.3f}")
        print(
            f"oFDR = P[H0|SR>SR_obs]    = {oFDR(SR=mu / sigma, SR0=SR0_adj, SR1=SR1 + SR0_adj, T=T, p_H1=p_H1, gamma3=gamma3, gamma4=gamma4, rho=rho, K=number_of_trials):.3f}"
        )
        ...
        print(
            f"oFDR = P[H0|SR>SR_obs] (K=1) = {oFDR(SR=mu / sigma, SR0=SR0_adj, SR1=SR1 + SR0_adj, T=T, p_H1=p_H1, gamma3=gamma3, gamma4=gamma4, rho=rho):.3f}"
        )
        ...
        print(
            f"σ_SR0                  = {math.sqrt(sharpe_ratio_variance(SR=SR0, gamma3=gamma3, gamma4=gamma4, rho=rho, T=T)):.3f}"
        )
        print(
            f"σ_SR1                  = {math.sqrt(sharpe_ratio_variance(SR=SR1, gamma3=gamma3, gamma4=gamma4, rho=rho, T=T)):.3f}"
        )
        ...
        print(
            f"σ_SR0                  = {math.sqrt(sharpe_ratio_variance(SR=SR0 + SR0_adj, gamma3=gamma3, gamma4=gamma4, rho=rho, T=T, K=number_of_trials)):.3f}"
        )
        print(
            f"σ_SR1                  = {math.sqrt(sharpe_ratio_variance(SR=SR1 + SR0_adj, gamma3=gamma3, gamma4=gamma4, rho=rho, T=T, K=number_of_trials)):.3f}"
        )
        print(f"α                      = {alpha_:.5f}")
        print(f"β                      = {beta_:.3f}")
        print(f"SR_c                   = {SR_c:.3f}")
        ...
        print(
            f"σ_SR0 (K=1)          = {math.sqrt(sharpe_ratio_variance(SR=SR0 + SR0_adj, gamma3=gamma3, gamma4=gamma4, rho=rho, T=T)):.3f}"
        )
        print(
            f"σ_SR1 (K=1)          = {math.sqrt(sharpe_ratio_variance(SR=SR1 + SR0_adj, gamma3=gamma3, gamma4=gamma4, rho=rho, T=T)):.3f}"
        )
